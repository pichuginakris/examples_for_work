...
def creating_bd(data, pointer):
    try:
        ...
        conn.commit()
        if pointer == 0:
            cursor.execute('DELETE FROM tables_joobleindia;')
            conn.commit()
        try:
            print(data)
            cursor.execute("INSERT INTO tables_joobleindia(vacancy, link_to_a_vacancy, salary, company, " +
            ...
            logging.error(
                'Exception of type {!s} in jooble_parser_india.py creating_bd(data, pointer): {!s}'.format(type(ex).__name__,  str(ex)))
            mes('Data base error in jooble_parser_india.py. See file logging_jooble_india.log for details')
        cursor.close()
        conn.close()
    except Exception as ex:
        logging.error(
            'Exception of type {!s} jooble_parser_india.py creating_bd(data, pointer): {!s}'.format(type(ex).__name__, str(ex)))
        mes('Data base error in jooble_parser_india.py. See file logging_jooble_india.log for details')
...
def searching(url, all_money):  # parsing
    logging.basicConfig(filename='logging_jooble_india.log', filemode='a',
    ...
    for i in range(1, 50):
        ...
        try:
            ...
            logging.error('Exception of type {!s} in jooble_parser_india searching(url, all_money): {!s}'.format(type(ex).__name__, str(ex)))
            mes('Wrong site url in jooble_parser_india.py. See file logging_jooble_india.log for details')
...
def main():
    ...
    try:
        ...
        reference = 'https://in.jooble.org/jobs?rt=1'
        searching(reference, all_money)
    except Exception as ex:
        logging.error(
            'Exception of type {!s} in jooble_parser_india.py main(): {!s}'.format(type(ex).__name__,
                                                                                         str(ex)))
        mes('Wrong site url in jooble_parser_india.py See file logging_jooble_india.log for details')
